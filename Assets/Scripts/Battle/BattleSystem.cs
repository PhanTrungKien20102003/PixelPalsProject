...
public class BattleSystem : MonoBehaviour
{
    ...
    IEnumerator ThrowPokeball(PokeballItems pokeballItems)
    {
        ...
        yield return dialogBox.TypeDialog($"{player.Name} use a {pokeballItems.Name.ToUpper()}!");
...
;           pokeball.transform.DOPunchRotation(new Vector3(0, 0, 10f), 0.8f).WaitForCompletion(); 
    ...
    {
        float a = (3 * pokemon.MaxHp - 2 * pokemon.HP) * pokemon.Base.CatchRate * pokeballItems.CatchRateModifier * ConditionsDB.GetStatusBonus(pokemon.Status) / (3 * pokemon.MaxHp);
